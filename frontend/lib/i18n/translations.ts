import { extendedEn, extendedHu } from '@/lib/i18n/extendedTranslations'
import { listingFormEn, listingFormHu } from '@/lib/i18n/listingFormTranslations'
import { listingDetailEn, listingDetailHu } from '@/lib/i18n/listingDetailTranslations'
import { bookingPagesEn, bookingPagesHu } from '@/lib/i18n/bookingPagesTranslations'

export type Locale = 'en' | 'hu'

const baseEn = {
  // Navbar
  'nav.browse': 'Browse',
  'nav.rentRequests': 'Request',
  'nav.postListing': 'Post Listing',
  'nav.dashboard': 'Dashboard',
  'nav.bookings': 'Bookings',
  'nav.messages': 'Messages',
  'nav.profile': 'Profile',
  'nav.login': 'Login',
  'nav.signUp': 'Sign Up',
  'nav.logout': 'Logout',
  'nav.toggleTheme': 'Toggle theme',
  'nav.language': 'Language',

  // Footer
  'footer.tagline': 'Rent what you need. Hire who you trust.',
  'footer.subtitle': 'The local marketplace for gear, spaces, and skilled people — now live in Budapest.',
  'footer.contactSupport': 'Contact support →',
  'footer.followUs': 'Follow us',
  'footer.forRenters': 'For Renters',
  'footer.browseListings': 'Browse Listings',
  'footer.howItWorks': 'How It Works',
  'footer.safetyTrust': 'Safety & Trust',
  'footer.renterResources': 'Renter resources',
  'footer.forOwners': 'For Owners',
  'footer.postAListing': 'Post a Listing',
  'footer.ownerResources': 'Owner resources',
  'footer.company': 'Company',
  'footer.aboutUs': 'About Us',
  'footer.contact': 'Contact',
  'footer.termsConditions': 'Terms & Conditions',
  'footer.privacyPolicy': 'Privacy Policy',
  'footer.rightsReserved': 'All rights reserved.',
  'footer.terms': 'Terms',
  'footer.privacy': 'Privacy',
  'footer.cookies': 'Cookies',

  // Hero
  'hero.nowLive': 'Now live in {region}',
  'hero.eyebrow': 'Access over ownership',
  'hero.titleLine1': 'Rent what you need.',
  'hero.titleLine2': 'Hire who you trust.',
  'hero.titleLine1Aria': 'Browse items to rent',
  'hero.titleLine2Aria': 'Browse people and services to hire',
  'hero.subtitle':
    'Why buy when you can borrow? Earn from what you already own. Items, spaces, and skills from your community — booked in minutes.',
  'hero.searchPlaceholder': 'Try: camera, drill, van, photographer…',
  'hero.searchAria': 'Search listings',
  'hero.searchButton': 'Search',
  'hero.trust1': 'Verified local listings',
  'hero.trust2': 'Secure in-app messaging',
  'hero.trust3': 'No platform fees',
  'hero.listCta': 'List something you own',
  'hero.howCta': 'See how Rhentify works',
  'hero.stat1Value': 'Minutes',
  'hero.stat1Label': 'to book nearby',
  'hero.stat2Value': '100%',
  'hero.stat2Label': 'local-first',
  'hero.stat3Value': '0 Ft',
  'hero.stat3Label': 'platform fee',

  // How it works
  'howItWorks.titlePrefix': 'How it',
  'howItWorks.titleHighlight': 'works',
  'howItWorks.subtitle': 'Four simple steps to rent or earn in {region}',
  'howItWorks.feeNote': 'No platform fees — agree details in chat, then book.',
  'howItWorks.forRenters': 'For renters',
  'howItWorks.forOwners': 'For owners',
  'howItWorks.step1Title': 'Search locally',
  'howItWorks.step1Desc':
    'Browse gear, spaces, and services near you — filter by category, district, or what you need.',
  'howItWorks.step2Title': 'Message & book',
  'howItWorks.step2Desc':
    'Ask questions in the app, agree dates and price, then confirm the booking together.',
  'howItWorks.step3Title': 'Meet or pick up',
  'howItWorks.step3Desc':
    'Collect the item, use the space, or welcome your service provider at the agreed time.',
  'howItWorks.step4Title': 'Review & repeat',
  'howItWorks.step4Desc':
    'Leave feedback after the rental — it helps the community and your next booking.',
  'howItWorks.owner1Title': 'List in minutes',
  'howItWorks.owner1Desc': 'Post gear, a space, or a service with photos, rates, and availability.',
  'howItWorks.owner2Title': 'Get requests',
  'howItWorks.owner2Desc': 'Renters find your listing, message you, and book when it suits you.',
  'howItWorks.owner3Title': 'Hand over & earn',
  'howItWorks.owner3Desc': 'Arrange pickup or a visit — payment terms stay between you and the renter.',
  'howItWorks.owner4Title': 'Build trust',
  'howItWorks.owner4Desc': 'Good reviews bring repeat bookings and a stronger local reputation.',
  'howItWorks.browseCta': 'Browse listings',
  'howItWorks.listCta': 'List something you own',
  'howItWorks.safetyLink': 'Safety & trust tips',

  // Profile — language
  'profile.myProfile': 'My Profile',
  'profile.languageTitle': 'Platform language',
  'profile.languageDesc':
    'Choose how the platform is shown. Navigation, bookings, messages, listings, and guides are available in Hungarian and English.',
  'profile.english': 'English',
  'profile.englishHint': 'English — full UI, guides, and listing flows',
  'profile.hungarian': 'Magyar',
  'profile.hungarianHint': 'Magyar — teljes felület, útmutatók és hirdetési folyamatok',
  'profile.languageChanged': 'Language updated',

  // Profile — existing (partial Phase 1)
  'profile.saveChanges': 'Save Changes',
  'profile.saving': 'Saving...',
  'profile.cancel': 'Cancel',
  'common.yes': 'Yes',
  'common.no': 'No',

  // Categories
  'category.browsePrefix': 'Browse by',
  'category.browseHighlight': 'Category',
  'category.subtitle':
    'Focused categories for renting in Budapest — tools, tech, baby gear, fashion, pets, and more.',
  'category.label': 'Category',
  'category.all': 'All categories',

  // Search
  'search.itemLabel': 'Item or service',
  'search.itemPlaceholder': 'Camera, drill, photographer…',
  'search.districtLabel': 'District',
  'search.districtPlaceholder': 'e.g. 9th district',
  'search.areaLabel': 'Area',
  'search.areaPlaceholder': 'e.g. Budapest, Hungary',
  'search.locationLabel': 'District or area',
  'search.locationPlaceholder': 'e.g. 9th district, Budapest, Hungary',
  'search.submit': 'Search',
  'search.browseCategories': 'Browse categories',
  'search.allCategories': 'All',

  'filters.title': 'Filters',
  'filters.active': '{count} active',
  'filters.toggleHide': 'Hide filters',
  'filters.toggleShow': 'Show filters',
  'filters.type': 'Listing type',
  'filters.allTypes': 'All types',
  'filters.items': 'Items',
  'filters.workers': 'Services & people',
  'filters.priceRange': 'Price per day',
  'filters.min': 'Min',
  'filters.max': 'Max',
  'filters.resetRefinements': 'Clear refinements',
  'filters.resetAll': 'Clear all filters',

  // Rent requests (feed)
  'feed.pageTitle': 'Rent requests',
  'feed.subtitleBefore12': 'See what people are looking to rent right now. When you publish, choose how long your post stays live —',
  'feed.hours12': '12 hours',
  'feed.subtitleOr': 'or',
  'feed.hours24': '24 hours',
  'feed.subtitleAfter24': '— then it disappears from rent requests.',
  'feed.formAria': 'Create a rent request',
  'feed.typeLabel': 'What are you looking for?',
  'feed.typeAria': 'Request type',
  'feed.typeItem': 'Item or gear',
  'feed.typeWorker': 'Person or service',
  'feed.typeHint': 'Items use pickup, delivery and deposit options. Service requests focus on timing and budget.',
  'feed.postHeadingItem': 'Post what you’d like to rent',
  'feed.postHeadingWorker': 'Post who you need to hire',
  'feed.postHeading': 'Post what you’d like to rent',
  'feed.titleLabelItem': 'What do you need to rent?',
  'feed.titleLabelWorker': 'Who or what service do you need?',
  'feed.titleLabel': 'What do you need?',
  'feed.titlePlaceholderItem': 'e.g. DSLR camera for a weekend shoot',
  'feed.titlePlaceholderWorker': 'e.g. plumber for a kitchen leak tonight',
  'feed.titlePlaceholder': 'e.g. DSLR camera for a weekend shoot or plumber for this evening',
  'feed.titleHintItem': 'Short name of the item you want to rent ({count}/200)',
  'feed.titleHintWorker': 'Role or service you need ({count}/200)',
  'feed.titleHint': 'Short name of the item or service you want to rent ({count}/200)',
  'feed.detailsPlaceholderItem': 'How long you need it, condition, brand, or any other requirements…',
  'feed.detailsPlaceholderWorker': 'Scope of work, skills needed, on-site vs remote, etc.',
  'feed.detailsPlaceholder': 'How long you need it, condition, brand, or any other requirements…',
  'feed.timingLabel': 'When do you need them?',
  'feed.timingPlaceholder': 'e.g. tonight from 6pm, Saturday morning, ~2 hours',
  'feed.timingHint': 'Date, time window, or estimated duration ({current}/{max})',
  'feed.budgetPlaceholderWorker': 'e.g. 8,000 HUF/hour for ~2 hours',
  'feed.budgetHintWorker': 'Hourly rate, fixed price, or total budget — your words. ({current}/{max})',
  'feed.areaHintWorker': 'Where the work happens. Fill any combination of district, city or country.',
  'feed.badgeItem': 'Item',
  'feed.badgeWorker': 'Service',
  'feed.filterAll': 'All',
  'feed.filterItems': 'Items',
  'feed.filterServices': 'Services',
  'feed.emptyFiltered': 'No active requests in this category.',
  'feed.detailsLabel': 'Details',
  'feed.optional': '(optional)',
  'feed.charCount': '{current}/{max} characters',
  'feed.areaLabel': 'Area',
  'feed.districtSr': 'District or neighbourhood',
  'feed.districtPlaceholder': 'District / neighbourhood',
  'feed.citySr': 'City',
  'feed.cityPlaceholder': 'City',
  'feed.countrySr': 'Country',
  'feed.countryPlaceholder': 'Country',
  'feed.areaHint': 'Helps nearby hosts notice your request. Fill any combination of district, city or country.',
  'feed.budgetLabel': 'Budget',
  'feed.budgetPlaceholder': 'e.g. 25,000 HUF total for 2 days',
  'feed.budgetHint': 'Rental price and duration only — use deposit options below for security terms. ({current}/{max})',
  'feed.depositLabel': 'Deposit preference',
  'feed.depositAria': 'Deposit preference',
  'feed.depositNone': 'Prefer no deposit',
  'feed.depositCash': 'Cash deposit OK',
  'feed.depositItem': 'Item deposit OK',
  'feed.depositFlexible': 'Discuss in chat',
  'feed.depositBadgeNone': 'No deposit',
  'feed.depositBadgeCash': 'Cash deposit',
  'feed.depositBadgeItem': 'Item deposit',
  'feed.depositBadgeFlexible': 'Flexible',
  'feed.depositCashPlaceholder': 'e.g. up to 10,000 HUF cash deposit',
  'feed.depositItemPlaceholder': 'e.g. ID or laptop as collateral',
  'feed.depositNoteHint': 'Optional detail for owners. ({current}/{max})',
  'feed.depositHint': 'Tap again to clear. Helps owners know what security you’re OK with before they reply.',
  'feed.deliveryLabel': 'Delivery or pickup',
  'feed.deliveryAria': 'Delivery preference',
  'feed.deliveryPickup': 'Pickup',
  'feed.deliveryDelivery': 'Delivery',
  'feed.deliveryEither': 'Either works',
  'feed.deliveryHint': 'Tap again to clear. Choose “Either works” if you’re flexible.',
  'feed.visibilityLabel': 'How long should it stay live?',
  'feed.visibilityAria': 'Post visibility duration',
  'feed.visibility12': '12 hours',
  'feed.visibility24': '24 hours',
  'feed.visibilityHint': 'Shorter posts (12h) for urgent needs; 24h gives owners more time to reply.',
  'feed.submitPosting': 'Posting…',
  'feed.submitPublish': 'Publish ({hours}h)',
  'feed.signInPrompt': 'Sign in to post what you’re looking to rent.',
  'feed.liveRequests': 'Live requests',
  'feed.loading': 'Loading rent requests…',
  'feed.empty': 'No active requests right now. Be the first to post.',
  'feed.expiresSoon': 'soon',
  'feed.expires': '{hours}h · Expires {when}',
  'feed.replyInMessages': 'Reply in Messages',
  'feed.replyModalTitle': 'Reply to live request',
  'feed.replyModalIntro': 'We’ll open your Messages thread with',
  'feed.replyModalPrompt': 'request posted',
  'feed.replyModalMid': '— the fixed prompt the renter sees (tagged',
  'feed.replyModalTag': 'live request reply',
  'feed.replyModalEnd': '). You won’t type a message here; you can continue in Messages after.',
  'feed.replyRequestLabel': 'Request:',
  'feed.loadingListings': 'Loading your listings…',
  'feed.noListings': 'You need an active listing to reply.',
  'feed.createListing': 'Create a listing',
  'feed.yourListing': 'Your listing',
  'feed.selectPlaceholder': 'Select…',
  'feed.cancel': 'Cancel',
  'feed.opening': 'Opening…',
  'feed.openMessages': 'Open Messages',
  'feed.toast.loadFailed': 'Could not load rent requests',
  'feed.toast.titleRequired': 'Add a short title for what you want to rent',
  'feed.toast.posted': 'Posted — visible for {hours} hours',
  'feed.toast.postFailed': 'Could not post — try signing in again',
  'feed.toast.listingsFailed': 'Could not load your listings',
  'feed.toast.pickListing': 'Choose one of your listings to reply with',
  'feed.toast.conversationOpened': 'Messages opened — “request posted” was added for the renter',
  'feed.toast.conversationFailed': 'Could not start conversation',

  // Resources hub
  'resources.hubTitle': 'Resources',
  'resources.hubSubtitle':
    'Separate guides for renters and owners in {region}. Choose the path that fits you.',
  'resources.renterTitle': 'Renter resources',
  'resources.renterDesc':
    'Browse listings, post rent requests, book dates, stay safe at pickup, and leave reviews.',
  'resources.renterCta': 'Open renter guides',
  'resources.ownerTitle': 'Owner resources',
  'resources.ownerDesc':
    'Create listings, price in HUF, reply to bookings and rent requests, and earn strong reviews.',
  'resources.ownerCta': 'Open owner guides',

  // Resource guide shell
  'resources.quickStart': 'Quick start',
  'resources.needHelp': 'Need more help?',
  'resources.needHelpDesc': 'Safety tips, support, and community updates.',
  'resources.safetyTrust': 'Safety & Trust',
  'resources.contactSupport': 'Contact support',
  'resources.followRhentify': 'Follow Rhentify',
} as const

const en = { ...baseEn, ...extendedEn, ...listingFormEn, ...listingDetailEn, ...bookingPagesEn } as const

export type TranslationKey = keyof typeof en

const baseHu: Record<keyof typeof baseEn, string> = {
  'nav.browse': 'Böngészés',
  'nav.rentRequests': 'Kérés',
  'nav.postListing': 'Hirdetés feladása',
  'nav.dashboard': 'Irányítópult',
  'nav.bookings': 'Foglalások',
  'nav.messages': 'Üzenetek',
  'nav.profile': 'Profil',
  'nav.login': 'Bejelentkezés',
  'nav.signUp': 'Regisztráció',
  'nav.logout': 'Kijelentkezés',
  'nav.toggleTheme': 'Téma váltása',
  'nav.language': 'Nyelv',

  'footer.tagline': 'Bérelj, amire szükséged van. Bízz abban, kit felveszel.',
  'footer.subtitle':
    'Helyi piactér felszerelésre, helyekre és szakemberekre — most Budapesten elérhető.',
  'footer.contactSupport': 'Ügyfélszolgálat →',
  'footer.followUs': 'Kövess minket',
  'footer.forRenters': 'Bérlőknek',
  'footer.browseListings': 'Hirdetések böngészése',
  'footer.howItWorks': 'Hogyan működik',
  'footer.safetyTrust': 'Biztonság és bizalom',
  'footer.renterResources': 'Bérlői útmutatók',
  'footer.forOwners': 'Tulajdonosoknak',
  'footer.postAListing': 'Hirdetés feladása',
  'footer.ownerResources': 'Tulajdonosi útmutatók',
  'footer.company': 'Cég',
  'footer.aboutUs': 'Rólunk',
  'footer.contact': 'Kapcsolat',
  'footer.termsConditions': 'Általános szerződési feltételek',
  'footer.privacyPolicy': 'Adatvédelmi irányelvek',
  'footer.rightsReserved': 'Minden jog fenntartva.',
  'footer.terms': 'Feltételek',
  'footer.privacy': 'Adatvédelem',
  'footer.cookies': 'Sütik',

  'hero.nowLive': 'Most elérhető: {region}',
  'hero.eyebrow': 'Hozzáférés a tulajdonlás helyett',
  'hero.titleLine1': 'Bérelj, amire szükséged van.',
  'hero.titleLine2': 'Bízz abban, kit felveszel.',
  'hero.titleLine1Aria': 'Tárgyak böngészése',
  'hero.titleLine2Aria': 'Szolgáltatások böngészése',
  'hero.subtitle':
    'Miért vennéd meg, ha bérelhetsz? Keress azzal, amid már megvan. Tárgyak, helyek és szolgáltatások a közösségedtől — percek alatt foglalható.',
  'hero.searchPlaceholder': 'Pl.: kamera, fúró, furgon, fotós…',
  'hero.searchAria': 'Hirdetések keresése',
  'hero.searchButton': 'Keresés',
  'hero.trust1': 'Ellenőrzött helyi hirdetések',
  'hero.trust2': 'Biztonságos alkalmazáson belüli üzenetküldés',
  'hero.trust3': 'Nincs platformdíj',
  'hero.listCta': 'Add fel, amid van',
  'hero.howCta': 'Így működik a Rhentify',
  'hero.stat1Value': 'Percek',
  'hero.stat1Label': 'a közeli foglaláshoz',
  'hero.stat2Value': '100%',
  'hero.stat2Label': 'helyi fókusz',
  'hero.stat3Value': '0 Ft',
  'hero.stat3Label': 'platformdíj',

  'howItWorks.titlePrefix': 'Hogyan',
  'howItWorks.titleHighlight': 'működik',
  'howItWorks.subtitle': 'Négy egyszerű lépés bérléshez vagy keresethez itt: {region}',
  'howItWorks.feeNote': 'Nincs platformdíj — egyeztess üzenetben, aztán foglalj.',
  'howItWorks.forRenters': 'Bérlőknek',
  'howItWorks.forOwners': 'Tulajdonosoknak',
  'howItWorks.step1Title': 'Keress helyben',
  'howItWorks.step1Desc':
    'Böngéssz felszerelés, helyiség és szolgáltatás hirdetéseket — szűrj kategória vagy kerület szerint.',
  'howItWorks.step2Title': 'Írj és foglalj',
  'howItWorks.step2Desc':
    'Kérdezz az alkalmazásban, egyezz meg időpontban és árban, majd erősítsd meg a foglalást.',
  'howItWorks.step3Title': 'Találkozz vagy vedd át',
  'howItWorks.step3Desc':
    'Vedd át a tárgyat, használd a helyet, vagy fogadd a szolgáltatót a megbeszélt időben.',
  'howItWorks.step4Title': 'Értékelj és térj vissza',
  'howItWorks.step4Desc':
    'Ossz meg visszajelzést a bérlés után — ez segít a közösségnek és a következő foglalásnak.',
  'howItWorks.owner1Title': 'Add fel percek alatt',
  'howItWorks.owner1Desc': 'Tárgy, hely vagy szolgáltatás — fotók, árak és elérhetőség.',
  'howItWorks.owner2Title': 'Kapj megkereséseket',
  'howItWorks.owner2Desc': 'A bérlők megtalálnak, üzennek, és foglalnak, ha neked is megfelel.',
  'howItWorks.owner3Title': 'Add át és keress',
  'howItWorks.owner3Desc': 'Egyeztess átvételt vagy látogatást — a fizetés köztetek marad.',
  'howItWorks.owner4Title': 'Építs bizalmat',
  'howItWorks.owner4Desc': 'A jó értékelések ismétlődő foglalásokat és erős helyi hírnevet hoznak.',
  'howItWorks.browseCta': 'Hirdetések böngészése',
  'howItWorks.listCta': 'Add fel, amid van',
  'howItWorks.safetyLink': 'Biztonság és bizalom',

  'profile.myProfile': 'Profilom',
  'profile.languageTitle': 'Platform nyelve',
  'profile.languageDesc':
    'Válaszd ki a platform nyelvét. A navigáció, foglalások, üzenetek, hirdetések és útmutatók magyarul és angolul is elérhetők.',
  'profile.english': 'English',
  'profile.englishHint': 'English — full UI, guides, and listing flows',
  'profile.hungarian': 'Magyar',
  'profile.hungarianHint': 'Magyar — teljes felület, útmutatók és hirdetési folyamatok',
  'profile.languageChanged': 'Nyelv frissítve',

  'profile.saveChanges': 'Mentés',
  'profile.saving': 'Mentés…',
  'profile.cancel': 'Mégse',
  'common.yes': 'Igen',
  'common.no': 'Nem',

  'category.browsePrefix': 'Böngészés',
  'category.browseHighlight': 'kategória szerint',
  'category.subtitle':
    'Budapesti bérléshez — szerszámok, tech, baba cuccok, divat, állatok és még sok más.',
  'category.label': 'Kategória',
  'category.all': 'Minden kategória',

  'search.itemLabel': 'Tárgy vagy szolgáltatás',
  'search.itemPlaceholder': 'Kamera, fúró, fotós…',
  'search.districtLabel': 'Kerület',
  'search.districtPlaceholder': 'pl. IX. kerület',
  'search.areaLabel': 'Terület',
  'search.areaPlaceholder': 'pl. Budapest, Magyarország',
  'search.locationLabel': 'Kerület vagy terület',
  'search.locationPlaceholder': 'pl. IX. kerület, Budapest',
  'search.submit': 'Keresés',
  'search.browseCategories': 'Kategóriák',
  'search.allCategories': 'Mind',

  'filters.title': 'Szűrők',
  'filters.active': '{count} aktív',
  'filters.toggleHide': 'Szűrők elrejtése',
  'filters.toggleShow': 'Szűrők megjelenítése',
  'filters.type': 'Hirdetés típusa',
  'filters.allTypes': 'Minden típus',
  'filters.items': 'Tárgyak',
  'filters.workers': 'Szolgáltatások és személyek',
  'filters.priceRange': 'Napi ár',
  'filters.min': 'Min',
  'filters.max': 'Max',
  'filters.resetRefinements': 'Szűkítés törlése',
  'filters.resetAll': 'Minden szűrő törlése',

  'feed.pageTitle': 'Bérleti igények',
  'feed.subtitleBefore12':
    'Nézd meg, mit szeretnének most bérelni. Közzétételkor válaszd ki, meddig maradjon látható —',
  'feed.hours12': '12 óra',
  'feed.subtitleOr': 'vagy',
  'feed.hours24': '24 óra',
  'feed.subtitleAfter24': '— utána eltűnik az igények közül.',
  'feed.formAria': 'Bérleti igény létrehozása',
  'feed.typeLabel': 'Mit keresel?',
  'feed.typeAria': 'Igény típusa',
  'feed.typeItem': 'Tárgy vagy felszerelés',
  'feed.typeWorker': 'Személy vagy szolgáltatás',
  'feed.typeHint': 'Tárgyaknál van átvétel, szállítás és kaució. Szolgáltatásnál időpont és költségkeret a lényeg.',
  'feed.postHeadingItem': 'Írd le, mit szeretnél bérelni',
  'feed.postHeadingWorker': 'Írd le, kit vagy milyen szolgáltatást keresel',
  'feed.postHeading': 'Írd le, mit szeretnél bérelni',
  'feed.titleLabelItem': 'Mit szeretnél bérelni?',
  'feed.titleLabelWorker': 'Kit vagy milyen szolgáltatást keresel?',
  'feed.titleLabel': 'Mire van szükséged?',
  'feed.titlePlaceholderItem': 'pl. DSLR kamera hétvégére',
  'feed.titlePlaceholderWorker': 'pl. vízvezeték-szerelő konyhai csőtörésre ma estére',
  'feed.titlePlaceholder': 'pl. DSLR kamera hétvégére vagy vízvezeték-szerelő ma estére',
  'feed.titleHintItem': 'A bérelni kívánt tárgy rövid neve ({count}/200)',
  'feed.titleHintWorker': 'Szükséges szerep vagy szolgáltatás ({count}/200)',
  'feed.titleHint': 'Rövid megnevezés — tárgy vagy szolgáltatás ({count}/200)',
  'feed.detailsPlaceholderItem': 'Meddig kell, milyen állapot, márka vagy egyéb elvárások…',
  'feed.detailsPlaceholderWorker': 'Feladat leírása, szükséges készségek, helyszínen vagy távolról, stb.',
  'feed.detailsPlaceholder': 'Meddig kell, milyen állapot, márka vagy egyéb elvárások…',
  'feed.timingLabel': 'Mikor kellene?',
  'feed.timingPlaceholder': 'pl. ma 18:00-tól, szombat reggel, ~2 óra',
  'feed.timingHint': 'Dátum, időablak vagy becsült időtartam ({current}/{max})',
  'feed.budgetPlaceholderWorker': 'pl. 8 000 Ft/óra, kb. 2 órára',
  'feed.budgetHintWorker': 'Óradíj, fix ár vagy teljes költségkeret — szabad szövegben. ({current}/{max})',
  'feed.areaHintWorker': 'Hol történik a munka. Tölts ki bármely kombinációt.',
  'feed.badgeItem': 'Tárgy',
  'feed.badgeWorker': 'Szolgáltatás',
  'feed.filterAll': 'Mind',
  'feed.filterItems': 'Tárgyak',
  'feed.filterServices': 'Szolgáltatások',
  'feed.emptyFiltered': 'Ebben a kategóriában nincs aktív igény.',
  'feed.detailsLabel': 'Részletek',
  'feed.optional': '(opcionális)',
  'feed.charCount': '{current}/{max} karakter',
  'feed.areaLabel': 'Terület',
  'feed.districtSr': 'Kerület vagy negyed',
  'feed.districtPlaceholder': 'Kerület / negyed',
  'feed.citySr': 'Város',
  'feed.cityPlaceholder': 'Város',
  'feed.countrySr': 'Ország',
  'feed.countryPlaceholder': 'Ország',
  'feed.areaHint': 'Segít, hogy a közeli tulajdonosok meglássák. Tölts ki bármely kombinációt.',
  'feed.budgetLabel': 'Költségkeret',
  'feed.budgetPlaceholder': 'pl. 25 000 Ft összesen 2 napra',
  'feed.budgetHint':
    'Csak bérleti díj és időtartam — a kauciót lent válaszd ki. ({current}/{max})',
  'feed.depositLabel': 'Kaució preferencia',
  'feed.depositAria': 'Kaució preferencia',
  'feed.depositNone': 'Inkább kaució nélkül',
  'feed.depositCash': 'Kp. kaució rendben',
  'feed.depositItem': 'Tárgyi kaució rendben',
  'feed.depositFlexible': 'Egyeztessünk chatben',
  'feed.depositBadgeNone': 'Kaució nélkül',
  'feed.depositBadgeCash': 'Kp. kaució',
  'feed.depositBadgeItem': 'Tárgyi kaució',
  'feed.depositBadgeFlexible': 'Rugalmas',
  'feed.depositCashPlaceholder': 'pl. max. 10 000 Ft kp. kaució',
  'feed.depositItemPlaceholder': 'pl. személyi vagy laptop fedezetként',
  'feed.depositNoteHint': 'Opcionális részlet a tulajdonosoknak. ({current}/{max})',
  'feed.depositHint':
    'Koppints újra a törléshez. Segít, ha milyen biztosíték megfelel neked.',
  'feed.deliveryLabel': 'Szállítás vagy átvétel',
  'feed.deliveryAria': 'Átvételi preferencia',
  'feed.deliveryPickup': 'Átvétel',
  'feed.deliveryDelivery': 'Szállítás',
  'feed.deliveryEither': 'Mindkettő jó',
  'feed.deliveryHint': 'Koppints újra a törléshez. Válaszd a „Mindkettő jó” opciót, ha rugalmas vagy.',
  'feed.visibilityLabel': 'Meddig maradjon aktív?',
  'feed.visibilityAria': 'Láthatóság időtartama',
  'feed.visibility12': '12 óra',
  'feed.visibility24': '24 óra',
  'feed.visibilityHint': '12 óra sürgős igényekhez; 24 óra több időt ad a tulajdonosoknak.',
  'feed.submitPosting': 'Közzététel…',
  'feed.submitPublish': 'Közzététel ({hours} óra)',
  'feed.signInPrompt': 'Jelentkezz be, hogy posztolhasd, mit keresel.',
  'feed.liveRequests': 'Aktív igények',
  'feed.loading': 'Igények betöltése…',
  'feed.empty': 'Jelenleg nincs aktív igény. Legyél te az első.',
  'feed.expiresSoon': 'hamarosan',
  'feed.expires': '{hours} ó · Lejár {when}',
  'feed.replyInMessages': 'Válasz üzenetben',
  'feed.replyModalTitle': 'Válasz az élő igényre',
  'feed.replyModalIntro': 'Megnyitjuk az üzenetek szálát ezzel:',
  'feed.replyModalPrompt': 'igény feladva',
  'feed.replyModalMid': '— a bérlő által látott fix szöveg (',
  'feed.replyModalTag': 'élő igény válasz',
  'feed.replyModalEnd': ' címkével). Itt nem írsz üzenetet; folytathatod az Üzenetekben.',
  'feed.replyRequestLabel': 'Igény:',
  'feed.loadingListings': 'Hirdetéseid betöltése…',
  'feed.noListings': 'Aktív hirdetés kell a válaszhoz.',
  'feed.createListing': 'Hirdetés létrehozása',
  'feed.yourListing': 'Hirdetésed',
  'feed.selectPlaceholder': 'Válassz…',
  'feed.cancel': 'Mégse',
  'feed.opening': 'Megnyitás…',
  'feed.openMessages': 'Üzenetek megnyitása',
  'feed.toast.loadFailed': 'Nem sikerült betölteni az igényeket',
  'feed.toast.titleRequired': 'Adj rövid címet annak, amit bérelni szeretnél',
  'feed.toast.posted': 'Közzétéve — {hours} óráig látható',
  'feed.toast.postFailed': 'Nem sikerült posztolni — jelentkezz be újra',
  'feed.toast.listingsFailed': 'Nem sikerült betölteni a hirdetéseidet',
  'feed.toast.pickListing': 'Válassz egy hirdetést a válaszhoz',
  'feed.toast.conversationOpened': 'Üzenetek megnyitva — „igény feladva” hozzáadva a bérlőnek',
  'feed.toast.conversationFailed': 'Nem sikerült elindítani a beszélgetést',

  'resources.hubTitle': 'Útmutatók',
  'resources.hubSubtitle':
    'Külön útmutatók bérlőknek és tulajdonosoknak — {region}. Válaszd a neked való utat.',
  'resources.renterTitle': 'Bérlői útmutatók',
  'resources.renterDesc':
    'Hirdetések böngészése, igény posztolása, foglalás, biztonságos átvétel és értékelés.',
  'resources.renterCta': 'Bérlői útmutatók megnyitása',
  'resources.ownerTitle': 'Tulajdonosi útmutatók',
  'resources.ownerDesc':
    'Hirdetés létrehozása, HUF árazás, foglalásokra és igényekre válasz, erős értékelések.',
  'resources.ownerCta': 'Tulajdonosi útmutatók megnyitása',

  'resources.quickStart': 'Gyors kezdés',
  'resources.needHelp': 'További segítség?',
  'resources.needHelpDesc': 'Biztonsági tippek, ügyfélszolgálat és közösségi hírek.',
  'resources.safetyTrust': 'Biztonság és bizalom',
  'resources.contactSupport': 'Ügyfélszolgálat',
  'resources.followRhentify': 'Kövess minket',
}

const hu: Record<TranslationKey, string> = {
  ...baseHu,
  ...extendedHu,
  ...listingFormHu,
  ...listingDetailHu,
  ...bookingPagesHu,
}

export const translations = { en, hu } as const

export function translate(
  locale: Locale,
  key: TranslationKey,
  vars?: Record<string, string>,
): string {
  let text = translations[locale][key] ?? translations.en[key] ?? key
  if (vars) {
    for (const [k, v] of Object.entries(vars)) {
      text = text.replace(`{${k}}`, v)
    }
  }
  return text
}

export const LOCALE_LABELS: Record<Locale, string> = {
  en: 'EN',
  hu: 'HU',
}

export const LOCALE_STORAGE_KEY = 'rhentify-locale'
